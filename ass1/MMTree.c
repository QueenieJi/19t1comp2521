...
MMTree MMTreeInsert (MMTree t, char *id, MailMessage mesg)
{
    ...
    MMTNode *prev = NULL;
    MMTNode *temp = t->root;
    // Iterate through the tree and find the correct place
    while (temp) {
        prev = temp;
        if (strcmp(id, temp->msgid) < 0) {
            temp = temp->left;
        } else {
            temp = temp->right;
        }
    }
    // Insert new node into the correct place
    MMTNode *new = newMMTNode (id, mesg);
    // If the new node is the first one
    if (!prev) {
        t->root = new;
    } else {
        if (strcmp(id, prev->msgid) < 0) {
            prev->left = new;
        } else {
            prev->right = new;
        }
