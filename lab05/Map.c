...
int connections (Map g, LocationID start, LocationID end, TransportID type[])
{
	...
	VList curr1 = g->connections[start];
	while (curr1 != NULL) {
	    VList curr2 = g->connections[end];
	    while (curr2 != NULL) {
	        if (curr2->v == curr1->v && curr2->type == BOAT && curr1->type == BOAT) {
	            type[counter] = BOAT;
	            counter ++;
	        }
	        curr2 = curr2->next;
	...
	return counter; // to keep the compiler happy
}
