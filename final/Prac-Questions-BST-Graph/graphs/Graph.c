 ...
 *
 */
int isCycle (Graph g, int s, int d, int *visited){
  visited[s] = 1;
  for (int i = 0; i < g->nV; i ++) {
    if (!g->edges[s][i]) continue;
    if (visited[i] == -1) return isCycle(g, i, s, visited);
    if (visited[i] && i != d) return 1;
  }
  return 0;
}
int hasCycle(Graph g) {
  ...
  for (int i = 0; i < g->nV; i++) {
    for (int j = 0; j < g->nV; j++) pred[j] = -1;
    if (isCycle(g, i, i, pred)) return 1;
    }
  // Stack s = newStack();
  // int i, j;
  // for (i = 0; i < g->nV; i++) {
  ...
  //     int k = popFrom(s);
  //     for (j = 0; j < g->nV; j++) {
  //       if (!g->edges[k][j]) continue;
  //       if (pred[j] == -1){
  //         pred[j] = k;
  //         pushOnto(s, j);
  //       } else if (j != i && pred[j] != -1 && pred[j] != k){
  //         return 1;
  //       }
  //     }
  //   }
...
int* within(Graph g, int s, int d, int *size) {
  ...
  int *final = malloc (sizeof(int) * g->nV);
  int i, j;
  ...
  int swap;
  // Sort the vertex with the same distance from the largest to the smallest
  for (i = 0; i < g->nV; i++) {
    swap = 0;
    for (j = 0; j + 1< g->nV; j++) {
      int m = final[j];
...
int *components(Graph g) {
  ...
  int i, j;
  int *com = malloc(sizeof(int) * g->nV);
  for (i = 0; i < g->nV; i++) {
    visited[i] = -1;
    com[i] = -1;
  }
